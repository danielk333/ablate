...
exp_scaled_heights = metablate.physics.alpha_beta.density_to_height_exponential_atm(
...
axes[0].plot(heights * 1e-3, heights * 1e-3, ls="--", label="Input = Scaled")
axes[0].set_xlabel("Initial Heights [km]")
axes[0].set_ylabel("Scaled heights [km]")
axes[0].grid("on")
axes[0].legend()

axes[1].semilogx(atm_density, exp_scaled_heights * 1e-3, "ob", label="Rescaled MSIS")
axes[1].semilogx(atm_density_exp, atm_scale_heights * 1e-3, "or", label="Rescaled exponential")
